...
class Webauthn {
	 ...
	 * @param string|null $fido_mds2_token - FIDO Metadata Service token
	 ...
	 * @param array $cred_params - Credential parameters (type and algorithm), see https://www.w3.org/TR/webauthn-2/#dictdef-publickeycredentialparameters
	 */
	public static function verify(string $client_data_json, array $att_obj, string $known_challenge, string $known_origin, string $known_rp_id, string $public_key = null, string $fido_mds2_token = null, int $last_sign_count = 0, bool $uv_required = false, bool $attestation_required = true, array $cred_params = [['type' => 'public-key', 'alg' => Cose::ALG_ES256]]): bool {
		$client_data = Json::decode($client_data_json); // §7.1 step 6 / §7.2 step 10
		...
		// check clientData.type - §7.1 step 7 / §7.2 step 11
		$client_data_expected_type = $public_key === null ? 'webauthn.create' : 'webauthn.get';
		...
		// check clientData.challenge against known challenge - §7.1 step 8 / §7.2 step 12
		...
		// check clientData.origin against known origin - §7.1 step 9 / §7.2 step 13
		...
		// hash client data - §7.1 step 11 / §7.2 step 19
		$client_data_hash = hash('sha256', $client_data_json, true);

		// check rpIdHash against known RP ID - §7.1 step 13 / §7.2 step 15
		...
		// check UP flag - §7.1 step 14 / §7.2 step 16
		...
		// check UV flag - §7.1 step 15 / §7.2 step 17
