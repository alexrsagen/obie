...
class Pem {
	...
	const LABEL_ENCRYPTED_PRIVATE_KEY = 'ENCRYPTED PRIVATE KEY';
	const LABEL_CERTIFICATE           = 'CERTIFICATE';
	const LABEL_ATTRIBUTE_CERTIFICATE = 'ATTRIBUTE CERTIFICATE';
	...
	public static function encode(string $data, string $label = self::LABEL_PUBLICKEY): string {
	...
	public static function decodeLabel(string $data): string|false {
		if (preg_match(self::REGEX, $data, $matches) !== 1) return false;
	...
	public static function getPreBoundary(string $label): string {
		return self::BOUNDARY_PREFIX_PRE . strtoupper($label) . self::BOUNDARY_SUFFIX;
	}

	public static function getPostBoundary(string $label): string {
